...
export default function OnlineMatch({code,role,settings,onExit}:Props){
  ...
  const seqRef=useRef(Date.now()*1000+Math.floor(Math.random()*1000)); const lastRemoteSeqRef=useRef<Record<string,number>>({}); const reconnectAttemptsRef=useRef(0)
  ...
  const [syncing,setSyncing]=useState(false); const [incompatiblePeers,setIncompatiblePeers]=useState(0)
  ...
  useEffect(()=>{
    ...
    let disposed=false;let retryTimer:number|undefined
    const clearRetry=()=>{if(retryTimer!==undefined)window.clearTimeout(retryTimer);retryTimer=undefined}
    const scheduleReconnect=(connectChannel:()=>void,immediate=false)=>{if(disposed)return;clearRetry();setConnected(false);setReconnecting(true);const attempt=reconnectAttemptsRef.current++;const delay=immediate?100:Math.min(8000,700*Math.pow(1.7,attempt));retryTimer=window.setTimeout(()=>{try{client.realtime.connect()}catch{};connectChannel()},delay)}
    const connectChannel=()=>{
      if(disposed)return
      const old=channelRef.current;channelRef.current=null;if(old)void client.removeChannel(old)
      const channel=client.channel(`kniffli:${normalizedCode}`,{config:{presence:{key:clientId},broadcast:{self:false,ack:true}}});channelRef.current=channel
      ...
      .subscribe(async status=>{if(disposed||channelRef.current!==channel)return;if(status==='SUBSCRIBED'){clearRetry();reconnectAttemptsRef.current=0;setConnected(true);setReconnecting(false);try{await channel.track({clientId,name:meRef.current.name,role,protocol:MULTIPLAYER_PROTOCOL,appVersion:APP_VERSION,joinedAt:Date.now()});if(startedRef.current){sendFullSync();sendAuthority();window.setTimeout(requestSync,100)}}catch{}}else if(status==='CHANNEL_ERROR'||status==='TIMED_OUT'||status==='CLOSED')scheduleReconnect(connectChannel)})
    }
    let lastWakeAt=0;const wake=()=>{if(disposed)return;const now=Date.now();if(now-lastWakeAt<1200)return;lastWakeAt=now;try{client.realtime.connect()}catch{};scheduleReconnect(connectChannel,true)};const visibility=()=>{if(document.visibilityState==='visible')wake()};const offline=()=>{setConnected(false);setReconnecting(true)}
    ...
    return()=>{disposed=true;clearRetry();document.removeEventListener('visibilitychange',visibility);window.removeEventListener('pageshow',wake);window.removeEventListener('online',wake);window.removeEventListener('offline',offline);const channel=channelRef.current;channelRef.current=null;if(channel)void client.removeChannel(channel)}
  ...
  const copyCode=async()=>{try{await navigator.clipboard.writeText(normalizedCode);softVibrate(10)}catch{}};const shareRoom=async()=>{const data={title:'Schmiddi & Schreier Spezial',text:`Komm in mein Kniffli-Spiel. Raum: ${normalizedCode}`,url:shareUrl};try{if(navigator.share)await navigator.share(data);else await navigator.clipboard.writeText(shareUrl)}catch{}}
  ...
  if(!started)return <main className="online-lobby screen-shell"><header className="simple-header"><button className="icon-button" onClick={onExit}><ChevronLeft/></button><span className="connection">{connected?<><Wifi size={15}/> verbunden</>:<><WifiOff size={15}/>{reconnecting?' verbindet neu …':' verbindet …'}</>}</span></header><span className="eyebrow">ONLINE · 2–4 iPHONES</span><h1>{role==='host'?'Dein Spielraum.':'Fast drin.'}</h1><p>{role==='host'?'Lade bis zu drei Mitspieler ein. Gespielt wird danach wie am Tisch immer nacheinander.':'Trag deinen Namen ein und warte, bis der Host startet.'}</p><label className="lobby-name-field"><span>DEIN NAME</span><input value={lobbyName} maxLength={16} onChange={e=>setLobbyName(e.target.value)} onBlur={()=>void updatePresenceName(lobbyName)} onKeyDown={e=>{if(e.key==='Enter')e.currentTarget.blur()}}/></label><div className="room-code-card"><small>RAUMCODE</small><strong>{normalizedCode}</strong><div><button onClick={copyCode}><Copy/>Code</button><button onClick={shareRoom}><Share2/>Teilen</button></div></div><div className="players-lobby multi-player-lobby"><div className="lobby-count"><Users/><span>{playerCount}/{MAX_PLAYERS} verbunden</span></div>{lobbyPlayers.map((p,index)=><div className="lobby-player" key={p.clientId}><span className="lobby-avatar">{p.name.slice(0,1).toUpperCase()}</span><b>{p.name}</b>{p.role==='host'&&<Crown size={14}/>}<small>{p.clientId===clientId?'DU':`SPIELER ${index+1}`}</small></div>)}</div>{roomFull&&<p className="micro-copy">Raum voll · maximal 4 Spieler.</p>}{incompatiblePeers>0&&<div className="waiting-banner">⚠️ {incompatiblePeers} Spieler nutzt eine ältere App-Version. Auf allen iPhones App schließen und neu öffnen.</div>}{role==='host'?<button className="primary-button" disabled={!connected||playerCount<2||incompatiblePeers>0} onClick={start}>MATCH MIT {playerCount} SPIELERN STARTEN</button>:<div className="waiting-pulse"><i/><span>Warte auf Host …</span></div>}<p className="micro-copy">Turn-based: Nur eine Person würfelt. Alle anderen sehen den Wurf live.</p></main>
